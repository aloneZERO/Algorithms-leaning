...
//		exhaustiveQuesion();
	 ...
	 * 穷举练习题
	 */
	public static void exhaustiveQuesion() {
		String n = "";
		int aim=0,m = 0;
		for(int a=1; a<=9; a++) {
			for(int b =0; b<=9; b++) {
				for(int c=0; c<=9; c++) {
					aim = Integer.parseInt(n+a+b+c);
					m = (int) Math.pow((double)aim,2);
					int result = condition(aim, m);
					if(result!=-1) System.out.println(result+" "+m);
				}
			}
		}
	} public static int condition(int aim, int m) {
		int target = aim;
		int[] a = new int[3];
		...
		return (ma[0]==a[0]&&ma[5]==a[1]&&ma[1]==ma[4]) ? aim : -1;
	}
